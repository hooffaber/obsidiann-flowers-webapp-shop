...
### Защита от доступа через обычный браузер (Telegram-only режим)

По умолчанию приложение доступно из любого браузера (удобно для разработки). Для production можно включить режим, когда приложение доступно **только из Telegram Mini App**.

#### Как работает защита

Middleware проверяет криптографическую подпись `X-Telegram-Init-Data` заголовка, используя HMAC-SHA256 с вашим bot token. Подделать эту подпись без знания токена **невозможно** (в отличие от простой проверки User-Agent).

#### Управление защитой

**Вариант 1: Django management команда** (рекомендуется)
...
```bash
# В backend/.env
ENFORCE_TELEGRAM_ONLY=true   # Включить защиту
ENFORCE_TELEGRAM_ONLY=false  # Выключить защиту
...
- ✅ Все API endpoints (`/api/v1/*`)
- ✅ Telegram Bot webhook (`/api/bot/*`)
- ❌ Django Admin (`/admin/`) - доступен всегда
- ❌ Health checks (`/health/`) - доступны всегда
- ❌ API документация (`/api/docs/`, `/api/redoc/`) - доступна админам
- ❌ Статические файлы (`/static/`, `/media/`)

#### Важно для production

```bash
# Обязательно включите защиту в production!
ENFORCE_TELEGRAM_ONLY=true
TELEGRAM_BOT_TOKEN=your_real_bot_token  # Проверьте что токен правильный
TELEGRAM_AUTH_TIMEOUT=86400             # 24 часа (по умолчанию)
```

### Защита API документации

API документация (`/api/docs/`, `/api/redoc/`) доступна **только администраторам**. Для доступа:

1. Войдите в Django admin: http://your-domain.com/admin/
2. Используйте аккаунт с правами `is_staff=True`
3. После входа документация откроется
